...
//// SimpleGoodTuringEstimator::SimpleGoodTuringEstimator - initialize ////
//
// INPUT: a "frequency of frequencies" counter, that might contain information such as:
//	1 => 945    [meaning: 945 words appeared exactly 1 time in the corpus]
//	2 => 123    [123 words appeared exactly 2 times]
//	3 => 14
//	4 => 3      
//	            [no words appeared exactly 5 times]
//	6 => 1      [1 word appeared exactly 6 times]
//	...
//	r[j] => n[j] for each j           [n[j] words appeared exactly r[j] times]
//	
// CALCULATION: the following variables are calculated:
//	rows = my_number_of_different_frequencies = the maximum value for "j".
//	r = an array that holds the frequencies. It should be sorted in ascending order. usually r starts with 1.
//	n = an array that holds the frequencies of the frequencies. It is usually sorted in descending order, but it is not required.
//		for each j, n[j] words appeared exactly r[j] times in the corpus.
//
// SEE ALSO: 	analyseInput, estimateNumberOfUnseenSpecies
//
...
//// SimpleGoodTuringEstimator::findBestFit ////
//
// CALCULATION: the following variables are calculated:
//	slope, intercept - regression coefficients, satisying:
//		log_Z  ~  intercept + slope * log_r
//
// SEE ALSO: smoothed
//
...
//// SimpleGoodTuringEstimator::smoothed ////
//
// INPUT: r_i, a frequency.
//
// RETURNS: the estimated value of log_Z, calculated by the formula above:
//		log_Z  ~  intercept + slope * log_r
//
// SEE ALSO: findBestFit
//
double SimpleGoodTuringEstimator::smoothed(int r_i) {
    return(exp(intercept + slope * log(r_i)));
}


//// SimpleGoodTuringEstimator::row ////
//
// INPUT: r_i, a frequency.
//
// CALCULATION: finds the frequency r_i in the array r.
//
// RETURNS: An index j to the array r, such that r[j] = r_i.
//	Or -1, if there is no such j.
//
// EXAMPLE: if r[0]=1, then row(1) will return 0.
//
// SEE ALSO: SimpleGoodTuringEstimator::SimpleGoodTuringEstimator
//
int SimpleGoodTuringEstimator::row(int r_i) {
    int j = 0;
    
    while (j < rows && r[j] < r_i)
            ++j;
    return((j < rows && r[j] == r_i) ? j : -1);
}
